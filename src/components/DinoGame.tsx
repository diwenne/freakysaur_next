...
const DinoGame = () => {
  ...
  const resetGame = () => {
    ...
    gameState.dino = new Dino(gameImages.DINO as DinoImages);
    gameState.obstacles = [];
    gameState.speed = 420;
    gameState.score = 0;
  ...
  const gameLoop = (timestamp: number) => {
    ...
    if (gameState.cloudTimer > 1.5) {
        ...
        // FIXED: Pass the specific cloud image, not the whole object
  ...
  const spawnObstacle = () => {
    if (!gameImages.OBSTACLES) return;
  ...
  useEffect(() => {
    ...
    loadAllAssets();
    return () => { isMounted = false; window.removeEventListener('keydown', handleInput); window.removeEventListener('keyup', handleInput); };
  }, []);
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      ...
      <div ref={webcamContainerRef} className="relative mb-4 border-2 border-gray-400 w-[900px] h-[240px] bg-gray-900 flex items-center justify-center overflow-hidden" >
...
class Dino { x = DINO_INITIAL_X; y = GROUND_Y; velY = 0; gravity = 2500; jumpSpeed = -900; isDucking = false; isAlive = true; images: DinoImages; currentImage: HTMLImageElement; width = 0; height = 0; runAnimTimer = 0; runAnimIndex = 0; runAnimRate = 0.09; jumpAnimTimer = 0; jumpAnimIndex = 0; jumpAnimRate = 0.1; 
    ...
    startJump() { if (this.isAlive && this.y + this.height >= GROUND_Y) { this.velY = this.jumpSpeed; this.jumpAnimIndex = 0; this.jumpAnimTimer = 0; } } 
