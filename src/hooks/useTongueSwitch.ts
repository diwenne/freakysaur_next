...
export const useTongueSwitch = () => {
    ...
    if (video.currentTime !== lastVideoTimeRef.current) {
      ...
      if (results.faceLandmarks?.length > 0) {
        ...
        if (innerLipPoints.length > 2) {
            const xs = innerLipPoints.map(p => p.x);
            const ys = innerLipPoints.map(p => p.y);
            const minX = Math.min(...xs), maxX = Math.max(...xs);
            const minY = Math.min(...ys), maxY = Math.max(...ys);

            const clampedMinX = Math.max(0, Math.floor(minX));
            const clampedMinY = Math.max(0, Math.floor(minY));
            const clampedMaxX = Math.min(w, Math.ceil(maxX));
            const clampedMaxY = Math.min(h, Math.ceil(maxY));
  ...
  const drawOverlay = (landmarks: NormalizedLandmark[], ctx: CanvasRenderingContext2D, width: number, height: number, isDetected: boolean, open: number, frac: number) => {
    ...
    for (const mark of relevantLandmarks) {
        minX = Math.min(minX, mark.x * width);
        maxX = Math.max(maxX, mark.x * width);
        minY = Math.min(minY, mark.y * height);
        maxY = Math.max(maxY, mark.y * height);
    }
